...
void PlotCode(){
    ...
    TH1F* h_0_12 = new TH1F("h_0_12", "Mass Distribution (2.1 < y < 2.4)", 30, 2.0, 3.6);
    TH1F* h_12_16 = new TH1F("h_12_16", "Mass Distribution (2.1 < y < 2.4)", 30, 2.0, 3.6);
    TH1F* h_16_19 = new TH1F("h_16_19", "Mass Distribution (1.6 < y < 1.9)", 30, 2.0, 3.6);
    TH1F* h_19_21 = new TH1F("h_19_21", "Mass Distribution (1.9 < y < 2.1)", 30, 2.0, 3.6);
    TH1F* h_21_24 = new TH1F("h_21_24", "Mass Distribution (2.1 < y < 2.4)", 30, 2.0, 3.6);
    ...
    for (int ievent = 0; ievent < tree->GetEntries(); ievent++) {
        ...
        for (size_t j = 0; j < cand_pdgId->size(); ++j) {
            if (cand_pdgId->at(j) == 443 && passHLT->at(17) && passHLT->at(18) && softmuon >= 1) {
...
auto FitAndDraw = [](TH1F* hist, const char* label, int canvasID) {
    RooRealVar x("m(J/#psi)", "m[GeV]", 2.0, 3.6);
    RooDataHist dh("dh", "dh", x, RooFit::Import(*hist));
    RooPlot* frame = x.frame();
    x.SetTitle(Form("Track+Track: Mass Distribution in %s", label));
