...
void PlotCode(){
    TFile *myFile = TFile::Open("~/Desktop/UPC/diMu_ana_mc.root");
    ...
    TH1F* h_0_12 = new TH1F("h_0_12", "Mass Distribution (0 < y < 1.2)", 10, 2.0, 3.6);
    TH1F* h_12_16 = new TH1F("h_12_16", "Mass Distribution (1.2 < y < 1.6)", 10, 2.0, 3.6);
    TH1F* h_16_19 = new TH1F("h_16_19", "Mass Distribution (1.6 < y < 1.9)", 10, 2.0, 3.6);
    TH1F* h_19_21 = new TH1F("h_19_21", "Mass Distribution (1.9 < y < 2.1)", 10, 2.0, 3.6);
    TH1F* h_21_24 = new TH1F("h_21_24", "Mass Distribution (2.1 < y < 2.4)", 10, 2.0, 3.6);
    ...
    for (int ievent = 0; ievent < tree->GetEntries(); ievent++) {
        ...
        // cout << "muon_softID" << muon_softID->size() << endl;
        // cout << "gen_eta" << gen_eta->size() << endl;

        // double softmuon = 0;
        // if (muon_softID->size() == 2){
        // for (int i = 0; i < muon_softID->size(); ++i) {
        //     if (muon_softID->at(i)){
        //         if (!isInsideExcludedRegion(gen_eta->at(i), gen_pT->at(i))){
        //             softmuon += 1; 
        //         }
        //     }
        // }
        ...
        for (size_t j = 0; j < cand_pdgId->size(); ++j) {
            if (cand_pdgId->at(j) == 443 && passHLT->at(17) && passHLT->at(18)) {
                ...
                else if (yval > 1.2 && yval <= 1.6) h_12_16->Fill(mass);
                else if (yval > 0 && yval <= 1.2) h_0_12->Fill(mass);
            }
        }
    }
...
auto FitAndDraw = [](TH1F* hist, const char* label, int canvasID) {
    RooRealVar pl("m(J/#psi)", "m[GeV]", 2.0, 3.6);
    RooDataHist dh("dh", "dh", pl, RooFit::Import(*hist));
    RooPlot* frame = pl.frame();
    pl.SetTitle(Form("Track+Track: Mass Distribution in %s", label));
    dh.plotOn(frame, RooFit::Name("hist"));

    RooRealVar meang("meang", "mean of Gaussian", 3.09, 3.08, 3.1);  // tighter bounds
    RooRealVar sigmag("sigmag", "std of Gaussian", 0.03, 0.005, 0.1); // start narrower
    RooGaussian gaussian("gaussian", "Gaussian PDF", pl, meang, sigmag);

    //Extend Gaussian with yield parameter
    RooRealVar ngaus("ngaus", "number of Gaussian signal events", 500, 0, 1e8);
    RooExtendPdf egaus("egaus", "extended Gaussian", gaussian, ngaus);
    RooAddPdf gmodel("gmodel", "Gaussian model", RooArgList(egaus));

    //Fit the extended Gaussian model
    RooFitResult *GaufitResult = gmodel.fitTo(dh, RooFit::Save(true));
    dh.plotOn(frame, RooFit::Name("histogramMass"));
    gmodel.plotOn(frame, RooFit::LineColor(kGreen+2), RooFit::Name("fit2"));

    double chi2Gaus = frame->chiSquare("fit2", "histogramMass");
    std::cout << "================ Gaussian Fit Results ====================" << std::endl;
    GaufitResult->Print("v");
    std::cout << "Chi^2 = " << chi2Gaus << std::endl;
    std::cout << "==========================================================" << std::endl;

    TCanvas* c = new TCanvas(Form("canvas%d", canvasID), label, 800, 600);
    frame->Draw();
 };
    int bin_low_0_12 = h_0_12->FindBin(2.9);
    int bin_high_0_12 = h_0_12->FindBin(3.2);
    std::cout << "[0 < y < 1.2] Integral [2.9, 3.2] = "
          << h_0_12->Integral(bin_low_0_12, bin_high_0_12) << std::endl;

    int bin_low_12_16 = h_12_16->FindBin(2.9);
    int bin_high_12_16 = h_12_16->FindBin(3.2);
    std::cout << "[1.2 < y < 1.6] Integral [2.9, 3.2] = "
          << h_12_16->Integral(bin_low_12_16, bin_high_12_16) << std::endl;
    ...
    FitAndDraw(h_12_16, "1.2 < y < 1.6", 4);
    FitAndDraw(h_0_12, "0 < y < 1.2", 5);
}
